...
function init() {
    if (s.$alerts.length) {
        ...
        const secondsSinceEpoch = Math.round(currentDate / 1000)
        ...
        s.$alerts.each(function (index, element) {
            ...
            if (startValue && endValue) {
                ...
                isInDateRange = startDate <= secondsSinceEpoch && secondsSinceEpoch <= endDate;
            ...
            if (isInDateRange) {
                if (
                    cookie(alertCookie) === undefined ||
                    cookie(alertCookie) !== "inactive" ||
                    isIE11
                ) {
...
function events() {
    $(".js-alert__dismiss").on("click", function (event) {
        event.preventDefault();
        const $this = $(this);
        cookie("STYXKEY-alert-" + $this.parent().data("id"), "inactive", {
            maxage: 1,
        });
