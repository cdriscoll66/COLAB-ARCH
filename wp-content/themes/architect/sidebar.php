...
<div id="sidebar" class="mdc-layout-grid__cell mdc-layout-grid__cell--span-4 mdc-layout-grid__cell--span-4-tablet mdc-layout-grid__cell--span-12-phone">
	...
	?>
		<div id="widget-area" class="widget-area" role="complementary">
			...
			?>
				<span class="edit-link"><a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>" class="badge badge-info"><?php esc_html_e( 'Edit', 'architect' ); ?></a></span><!-- Show Edit Widget link -->
			<?php
				endif;
			?>
		</div><!-- /.widget-area -->
		...
		<div class="sidebar-nav">
			<div id="primary-two" class="widget-area">
				<?php
					$output = '<ul class="mdc-list recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							$output .= '<li><h3>' . esc_html__( 'Recent Posts', 'architect' ) . '</h3></li>';
							while ( $recentposts_query->have_posts() ) :
								...
								$output .= '<li class="mdc-list-item">';
									$output .= '<h4><a href="' . esc_url( get_the_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'architect' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' .  apply_filters( 'the_title', get_the_title() ) . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
						wp_reset_postdata();
				...
				<ul class="categories mdc-list">
					<li><h3 class="border-bottom"><?php esc_html_e( 'Categories', 'architect' ); ?></h3></li>
					<?php
					...
					?>
							<li>&nbsp;</li>
							<li><a href="<?php echo esc_url( get_the_permalink( get_option( 'page_for_posts' ) ) ); ?>" class="mdc-button mdc-button--stroked"><?php esc_html_e( 'more', 'architect' ); ?></a></li>
					<?php
						endif;
					?>
				</ul>
